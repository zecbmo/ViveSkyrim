...
m12 will be part of idenity matrix
m13
m14
m15
...
inline Vector4 Matrix4::GetRotation()
{

	Vector4 q;
	q.w = sqrt(std::max(0.0f, 1 + this->m[0] + this->m[5] + this->m[10])) / 2;
	q.x = sqrt(std::max(0.0f, 1 + this->m[0] - this->m[5] - this->m[10])) / 2;
	q.y = sqrt(std::max(0.0f, 1 - this->m[0] + this->m[5] - this->m[10])) / 2;
	q.z = sqrt(std::max(0.0f, 1 - this->m[0] - this->m[5] + this->m[10])) / 2;
	q.x = _copysignf(q.x, this->m[9] - this->m[6]);
	q.y = _copysignf(q.y, this->m[2] - this->m[8]);
	q.z = _copysignf(q.z, this->m[4] - this->m[1]);
	return q;
}
